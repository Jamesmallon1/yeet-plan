...
2. **Protocol fees → 85% buyback + burn (immutable), 15% treasury.** `YeetBuyback` receives every protocol fee (launchpad and hook both `sweepFees()` into it; anyone can call). A fixed `BURN_SHARE_BPS = 8500` is reserved for burns with no withdraw path; the other 15% accrues and is pulled by the `treasury` address (owner-settable, defaults to owner). `setToken` works exactly once. `execute()` (permissionless; keeper) buys the protocol token from the curve while it is on the curve, from its Uniswap v4 pool after graduation, and burns everything. Protocol buybacks pay no protocol fee on themselves (fees on fees), but do pay the token's dividend. Mainnet order: deploy stack → launch the protocol token → `setToken` from the owner wallet.
...
Testnet lite stack protocol token: YEET `0xa8565fd7e62bd87829d4152e18342a2ebfa37442` (bound to the lite buyback `0x7C55A6EcFA408D4244C368A62945FCE71a6ca209`; stacks redeployed 9 Sep for the 85/15 change). Docs decision: public docs state a 5 s snipe window; chain enforces 3 s. 41 tests pass. Keeper key = deployer on testnet.
